import { closeAlerts, generateGUID, Logger, NetworkType, P2PPairingRequest, PostMessagePairingRequest, Serializer, windowRef } from "@airgap/beacon-sdk"
...
export const preparePairingAlert = async (
  ...
  await info.walletLists.filter(walletList => walletList.type === "ios")[0].wallets.filter(wallet => wallet.key === autonomyWalletKey)[0].clickHandler();
}