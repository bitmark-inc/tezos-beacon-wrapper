...
import { DAppClientOptions, PostMessageTransport, NetworkType, StorageKey } from '@airgap/beacon-dapp';
 ...
 */
export class DAppClientWrapped extends Client {
  /**
  ...
  protected isAutonomy: boolean = false;
   ...
   */
  protected async prepareBeforeAutonomyRequestPermission() {
    const autonomyName = "Autonomy";
    const activeAccount = await this.getActiveAccount()
    if (activeAccount) {
      const peerOfActiveAccount = await this.getPeer(activeAccount);
